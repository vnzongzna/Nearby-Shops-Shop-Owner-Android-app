        ...
        call.enqueue(new Callback<ResponseBody>() {
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                ...
                if(response.code()==200)
                {
                    ...
                    if(user.getRt_registration_mode()== User.REGISTRATION_MODE_PHONE)
                    {
                        phone.setError("An account already exist with that phone ... use another phone or reset your password !");
                    }
                    else if(user.getRt_registration_mode()== User.REGISTRATION_MODE_EMAIL)
                    {
                        email.setError("An account already exist with that e-mail ... use another email or reset your password !");
                    }
